...
class FakeLogger:
    def __init__(self):
        ...
        class _H(logging.Handler):
            ...
            def emit(self, rec): self.outer.records.append(rec)
...
@pytest.fixture
def logger(): return FakeLogger()

def test_internet_ok_logs_when_circuit_open(monkeypatch, logger):
    class FakeAppState:
        def circuit_blocked(self): return True
        def rate_limit_allow(self, _): pytest.fail("should not call rate_limit_allow")
...
def test_internet_ok_logs_when_rate_limited(monkeypatch, logger):
    class FakeAppState:
        def circuit_blocked(self): return False
        def rate_limit_allow(self, _): return False
    monkeypatch.setattr("lite_upgrade.AppState", FakeAppState)
