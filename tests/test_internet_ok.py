...
def test_internet_ok_false_when_circuit_blocked(monkeypatch, logger):
    class FakeAppState:
        ...
        def circuit_blocked(self) -> bool: return True
...
def test_internet_ok_false_when_rate_limit_disallows(monkeypatch, logger):
    class FakeAppState:
        def __init__(self): pass
        def circuit_blocked(self) -> bool: return False
        def rate_limit_allow(self, logger) -> bool: return False
...
def test_internet_ok_true_when_all_good(monkeypatch, logger):
    class FakeAppState:
        def __init__(self): pass
        def circuit_blocked(self) -> bool: return False
        def rate_limit_allow(self, logger) -> bool: return True
